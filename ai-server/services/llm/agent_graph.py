...
**返回值**：结构化 JSON，包含 `results` 数组。每个结果包含：
- `answer`：网页摘要内容
- `citations`：引用来源数组，每个元素包含 `title`（网页标题）和 `url`（链接地址）
- `images`：相关图片链接数组

**引用规范**（回答中必须遵守）：
- 正文中使用 `[index]` 上标形式引用来源，例如：`OpenAI 发布了新模型[1]`
- 回答末尾必须列出完整引用来源，格式为 markdown 链接：`[index] [网页标题](URL)`
